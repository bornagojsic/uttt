...
def handle_move():
    ...
    if request.method == 'POST':
        ...
        num_iters = int(request.json['numIters'])
        ...
        if not game_id in games.keys():
            ...
            clear_games()
            first_replys = open("first_replys.json", "r")
            replys = json.load(first_replys)
            first_replys.close()
            try:
                reply = replys[move]
                reply_move = reply[0]
                game_run = games[game_id]["game_run"]
                return {'move': reply_move, 'boardWinners': game_run.game.won, 'isOver': False, 'numItersRecieved': num_iters}
            except Exception:
                pass
        ...
        global num_sims, hide_evaluations
        ...
        return {'move': move_keys_inv[ai_move[0]], 'boardWinners': game_run.game.won, 'isOver': is_over, 'numItersRecieved': num_iters}
