...
class MCTS(Game): #Monte Carlo Tree Search Algorithm
    ...
    def UCB1(self, node, c=C_PARAMETER): #Upper Confidence Bound 1
        ...
        for i in node.children:
            ...
            ret.append(wr + c * math.sqrt(math.log(node.visits)/temp.visits))
        return ret
    ...
    def backpropagation_(self, node, result): #Inplace; modifies the values of the nodes with respect to the result of the simulation
        ...
        if node.parent_move == -1:
            if result == 1:
                node.wins += 1
            elif result == 0:
                node.wins += 0.5
        elif node.parent_move == 1:
            if result == -1:
                node.wins += 1
            elif result == 0:
                node.wins += 0.5
...
class Run(MCTS):
    ...
    def play_game(self): #Game playing
        ...
        difficulty = int(input("Enter number of iterations of MCTS: "))
        prev_player = 1
